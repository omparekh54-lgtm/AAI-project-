...
def test_environment_advances_and_returns_ppt_observations():
    env = CabDispatchEnv(seed=7, fleet_size=8, day_minutes=20)
    observations = env.reset()
    assert len(observations) == 8
    assert all(len(state) == 16 for state in observations.values())
    assert env.day_minutes == 20
...
def test_dqn_can_select_a_valid_action():
    ...
    cab = env.cabs[0]
    action = agent.act(observations[cab.cab_id], env.valid_actions(cab), explore=False)
    assert action in env.valid_actions(cab)


def test_simulation_produces_research_metrics():
    env = CabDispatchEnv(seed=13, fleet_size=8, day_minutes=60)
    ...
    assert 0.0 <= result["earnings_fairness"] <= 1.0
    assert result["avg_pickup_distance"] >= 0.0

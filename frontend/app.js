...
function resultFor(method, fleet=8){
  const rows = research.results || [];
  return rows.find(r => r.method === method && Number(r.fleet_size) === fleet) || rows.find(r => r.method === method) || null;
}
function seedCabs(n){ cabs=Array.from({length:n},(_,i)=>({id:i,x:Math.floor(i/5),y:i%5,status:'IDLE',action:'WAIT'})); }
function requests(){
  const peak = demand.value === 'Peak';
  const airport = demand.value === 'Airport-heavy';
  const count = peak ? 10 : airport ? 8 : 5;
  return Array.from({length:count},(_,i)=>({x:(i*3+minute)%5,y:(i*2+1)%5,hot:airport || i%3===0}));
}
function render(){
  city.innerHTML=''; const reqs=requests();
  ...
  const h=Math.floor((minute%1440)/60), m=minute%60;
  document.getElementById('clock').textContent=`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`;
  updateInspector();
}
function moveCab(c){
  if(c.status!=='IDLE') return;
  const req=requests().sort((a,b)=>(Math.abs(c.x-a.x)+Math.abs(c.y-a.y))-(Math.abs(c.x-b.x)+Math.abs(c.y-b.y)))[0];
  if(algorithm.value==='Nearest Cab' && req && Math.abs(c.x-req.x)+Math.abs(c.y-req.y)<=3){c.status='ON TRIP';c.action='ACCEPT RIDE';return;}
  if(algorithm.value==='Zone Balancing' && req){if(Math.abs(c.x-req.x)>Math.abs(c.y-req.y))c.x+=Math.sign(req.x-c.x);else c.y+=Math.sign(req.y-c.y);c.action='REPOSITION';return;}
  if(req && Math.random()<0.52){c.status='ON TRIP';c.action='ACCEPT RIDE';return;}
  const dirs=[[1,0,'SOUTH'],[-1,0,'NORTH'],[0,1,'EAST'],[0,-1,'WEST']], d=dirs[Math.floor(Math.random()*dirs.length)];
...
function updateInspector(){
  ...
  const zone=c.x*5+c.y; document.getElementById('agentZone').textContent=`${zone} · ${zoneName(c.x,c.y)}`;
  ...
  ['X '+c.x,'Y '+c.y,'T '+String(minute).padStart(4,'0'),'IDLE '+(c.status==='IDLE'?1:0),`REQ ${nearby}`,`IDLE FLEET ${cabs.filter(x=>x.status==='IDLE').length}`,`DEMAND ${demand.value}`,'NEIGHBOUR VIEW'].forEach(t=>{const s=document.createElement('span');s.textContent=t;grid.appendChild(s)});
...
function renderResearchMetrics(){
  ...
  updateMetric('waitMetric',`${Number(r.avg_wait_mean).toFixed(2)} min`);
  updateMetric('serviceMetric',`${(Number(r.service_rate_mean)*100).toFixed(1)}%`);
  updateMetric('cancelMetric',`${(100-Number(r.service_rate_mean)*100).toFixed(1)}%`);
  updateMetric('emptyMetric',`${(Number(r.empty_driving_ratio_mean)*100).toFixed(1)}%`);
  updateMetric('utilMetric',`${(Number(r.utilisation_mean)*100).toFixed(1)}%`);
  updateMetric('earnMetric',`₹${Number(r.earnings_mean).toFixed(0)}`);
}
function drawChart(){
  const c=document.getElementById('rewardChart');if(!c)return;const ctx=c.getContext('2d'),dpr=devicePixelRatio||1,w=c.clientWidth||500,h=c.clientHeight||240;c.width=w*dpr;c.height=h*dpr;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);
  ctx.strokeStyle='#20312b';ctx.lineWidth=1;for(let i=1;i<5;i++){const y=i*h/5;ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke()}
  const rewards=curve.rewards||[];if(!rewards.length)return;const lo=Math.min(...rewards),hi=Math.max(...rewards),range=Math.max(1,hi-lo);ctx.beginPath();rewards.forEach((v,i)=>{const x=(i/(rewards.length-1))*w,y=h-((v-lo)/range*.78+.1)*h;i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.strokeStyle='#a8ff72';ctx.lineWidth=2;ctx.stroke();ctx.fillStyle='#8ea29c';ctx.font='10px DM Mono';ctx.fillText(`ep 1`,8,h-4);ctx.fillText(`ep ${rewards.length}`,w-65,h-4);
}
function drawBars(){
  const box=document.getElementById('barChart');if(!box)return;box.innerHTML='';
  const rows=[['Avg wait',r=>Number(r.avg_wait_mean),' min'],['Service rate',r=>Number(r.service_rate_mean)*100,'%'],['Empty driving',r=>Number(r.empty_driving_ratio_mean)*100,'%'],['Earnings',r=>Number(r.earnings_mean),'']];
  rows.forEach(([label,get,suffix])=>{const vals=['Nearest Cab','Zone Balancing','Parameter-shared DQN'].map(m=>{const r=resultFor(m,8);return r?get(r):0});const max=Math.max(...vals)||1;const r=resultFor('Parameter-shared DQN',8);const value=r?get(r):0;const block=document.createElement('div');block.className='barline';block.innerHTML=`<span>${label}</span><div class="bar"><i style="width:${Math.max(4,Math.round((value/max)*100))}%"></i></div><b>${value.toFixed(label==='Earnings'?0:1)}${suffix}</b>`;box.appendChild(block)});
}
...
demand.onchange=render;window.onresize=drawChart;
